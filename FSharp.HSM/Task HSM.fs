...
type IStateMachine<'state, 'event, 'output, 'data> =
    ...
    abstract member Init: 'state -> Task
    ...
    abstract member Fire: 'event -> Task
    abstract member Fire: 'event * 'data -> Task

///Holds transition information for state
type Transition<'state, 'event, 'output, 'data> =
    { Event: 'event
      Guard: unit -> bool Task
      Raises: 'output option
      NextState: 'data option -> 'state option Task }
...
module internal Core =
    ...
    //Helpers
    let exit f state : Task = task {
        do! state.Exit()
        List.iter f state.ExitRaises
    }

    let enter f state : Task = task {
        do! state.Enter()
    ...
    type internal StateMachine<'state, 'event, 'output, 'data when 'state: equality and 'state: comparison and 'event: equality and 'event: comparison>(stateList: StateConfig<'state, 'event, 'output, 'data> list) =
        ...
        let rec transition newState = task {
            ...
            do! exitState trigger current newStateConfig
            do! enterState trigger current newStateConfig

            do! newStateConfig.Enter()
            ...
            | None -> return ()
            | Some x -> return! transition x
        }

        let tryTransition data trans = task {
            let! guard = trans.Guard()
            if guard then
                Option.iter trigger trans.Raises
                match! trans.NextState data with
                | Some state -> return! transition state
                | None -> ()
        ...
        interface IStateMachine<'state, 'event, 'output, 'data> with
            ...
            member this.Init state = task {
                started <- true
                current <- configs.[state]

                do! current.Parents
                    |> Seq.rev
                    |> Seq.map (enter trigger)
                    |> Task.WhenAll
            ...
            member this.Fire(event) = task {
                if not started then
                    ()
                else
                    match findTransition event current current.Parents with
                    | Some trans -> return! tryTransition None trans
                    | None -> ()
            }

            ///Fire an event with data
            member this.Fire(event, data) = task {
                if not started then
                    ()
                else
                    match findTransition event current current.Parents with
                    | Some trans -> return! tryTransition (Some data) trans
                    | None -> ()
            }

let private noop = fun () -> Task.CompletedTask
let private alwaysTrue = fun () -> Task.FromResult true
...
let on event endState state =
    let transition =
        { Event = event
          NextState = (fun _ -> Some(endState) |> Task.FromResult)
          Guard = alwaysTrue
          Raises = None }
...
///Sets an event handler (with or without data) which returns the new state to transition to
let handleData event f state =
    let trans =
        { Event = event
          NextState = function None -> None |> Task.FromResult | Some data -> f data
          Guard = alwaysTrue
          Raises = None }
